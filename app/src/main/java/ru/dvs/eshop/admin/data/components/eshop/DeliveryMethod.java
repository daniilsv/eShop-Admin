...
/**
 * Created by MSI1 on 26.08.2016.
 */
public class DeliveryMethod extends Model {
    /*
    id
    original_id
    is_enabled
    ...
    private DeliveryMethod(Cursor c) {
        super();
        ...
        price = c.getFloat(c.getColumnIndex("price"));
        ...
        try {
            ...
            if (icon_keys != null)
                while (icon_keys.hasNext()) {
                    ...
                    Drawable d = Drawable.createFromPath(Core.getStorageDir() + "/icons/delivery_methods/" + original_id + "/" + tmp[tmp.length - 1]);
                    icons.put(key, d);
                }
    ...
    public static ArrayList<DeliveryMethod> getDeliveryMethods() {
        ArrayList<DeliveryMethod> ret = new ArrayList<>();
        Cursor c = DB.query("com_eshop_delivery_methods", null, null, null, null, null, "ordering ASC");
        if (c == null || !c.moveToFirst()) return ret;
        do {
            ret.add(new DeliveryMethod(c));
        } while (c.moveToNext());
        c.close();
        return ret;
    }
    ...
    public static void loadFromSite() {
        ...
        task.put("method", "get_delivery_method");
        task.execute();
    }
    ...
    private static void parseResponse(String response) {
        ...
        try {
            ...
            while (keys.hasNext()) {
                ...
                if (icon_keys != null)
                    while (icon_keys.hasNext()) {
                        ...
                        if (key.equals("big") || key.equals("small") || key.equals("original")) {
                            ...
                            icons_href.put(key, href);
                            String tmp[] = href.split("/");
                            new FILEQuery(site.host + href, Core.getStorageDir() + "/icons/delivery_methods/" + orig_id + "/" + tmp[tmp.length - 1]).execute();
                        }
                    }
                ...
                Cursor bd_item = DB.query("com_eshop_delivery_methods", null, "original_id=" + orig_id, null, null, null, null);
                if (bd_item != null && bd_item.moveToFirst()) {//Элемент в таблице есть - обновляем данные
                    DB.update("com_eshop_delivery_methods", bd_item.getInt(bd_item.getColumnIndex("id")), map);
                } else {//Элемента в таблиц нет - добавляем его
                    DB.insert("com_eshop_delivery_methods", map);
                }
...
/*
{
    ...
    "response":{
            ...
            "description":null,
            "icon":{
                "original":"000\/u1\/001\/d19f4674.jpg",
                "micro":"000\/u1\/001\/88b138c6.jpg",
                "small":"000\/u1\/001\/d7b589e4.jpg",
                "normal":"000\/u1\/001\/e38ed4bb.jpg",
                "big":"000\/u1\/001\/7fca732c.jpg",
                "wysiwyg_markitup":"000\/u1\/001\/0e6489f1.jpg",
                "wysiwyg_redactor":"000\/u1\/001\/28c417db.jpg",
                "wysiwyg_live":"000\/u1\/001\/c4873282.jpg"},
            "price":"0"}
    }
 */